...
## Локальная разработка и авторизация

OAuth HH работает только тогда, когда адреса из настроек действительно ведут на запущенную серверную часть JobRadar. Если бот запущен на ноутбуке, а `APP_BASE_URL` или `HH_REDIRECT_URI` указывают на Vercel, авторизация оборвётся: Vercel отдаст страницу лендинга и не передаст `/auth/hh/start` или `/auth/hh/callback` в локальный `aiohttp`-сервер на порту `8080`.

Для локальной разработки используйте один из двух вариантов.

Вариант 1: публичный туннель через ngrok.

```bash
ngrok http 8080
```

После запуска ngrok укажите выданный HTTPS-адрес в `.env`:

```env
APP_BASE_URL=https://your-ngrok-domain.ngrok-free.app
HH_REDIRECT_URI=https://your-ngrok-domain.ngrok-free.app/auth/hh/callback
```

Тот же `HH_REDIRECT_URI` нужно прописать в настройках приложения на `dev.hh.ru`. После этого кнопка в Telegram будет вести в локальную серверную часть через публичный адрес туннеля, а HH сможет вернуть пользователя на `/auth/hh/callback`.

Вариант 2: полностью локальный callback.

```env
APP_BASE_URL=http://127.0.0.1:8080
HH_REDIRECT_URI=http://127.0.0.1:8080/auth/hh/callback
```

Такой же адрес возврата нужно указать в приложении на `dev.hh.ru`. Этот вариант подходит только для проверки с той же машины и зависит от того, принимает ли HH локальный HTTP-адрес в настройках приложения.

Быстрая диагностика серверной части:

```text
http://127.0.0.1:8080/
```

Если сервер запущен, страница покажет: «Бэкенд JobRadar запущен. Для авторизации перейдите в бота.»
